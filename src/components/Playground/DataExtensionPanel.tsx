      ...
      <div className={styles.deList}>
        {dataExtensions.map((de, i) => (
          <div key={i} className={styles.deItem}>
            ...
            <button className={styles.removeBtn} onClick={() => removeDE(i)} title="Remover DE">×</button>
          </div>
        ))}
        ...
        <div className={styles.attrRow}>
          <input
            ...
            placeholder="Nome da nova Data Extension"
            value={newDeName}
        ...
        <div className={styles.deTable}>
          <div className={styles.deTableActions}>
            ...
            <button
              ...
              style={{ color: importOpen ? 'var(--ifm-color-primary)' : undefined }}
            >
          ...
          {importOpen && (
            <div className={styles.csvImport}>
              <p className={styles.csvHint}>
                Aceita dados <strong>separados por tabulação</strong> (Excel/Google Sheets) ou <strong>por vírgula</strong> (CSV). 
                A primeira linha contém os cabeçalhos das colunas.
              </p>
              <textarea
                className={styles.csvTextarea}
                placeholder={'Nome\tEmail\tCidade\nFabi\tfabi@exemplo.com\tPoços de Caldas'}
                value={csvText}
              ...
              <div className={styles.csvActions}>
                ...
                <button className={styles.csvImportBtn} onClick={handleImport}>Importar</button>
              </div>
          ...
          {cols.length === 0 ? (
            <p className={styles.deEmpty}>
              Ainda não há colunas. Digite o nome da coluna abaixo e dê Enter, ou use o Importar.
            </p>
          ) : (
                    ...
                    {cols.map(c => (
                      <th key={c}>
                        <div className={styles.colHeader}>
                          <span>{c}</span>
                          <button className={styles.removeColBtn} onClick={() => removeColumn(c)} title={`Remover ${c}`}>×</button>
                        </div>
                      </th>
                    ))}
                    <th style={{ width: '24px' }} />
                ...
                <tbody>
                  {selected.rows.map((row, ri) => (
                    <tr key={ri}>
                      {cols.map(c => (
                        <td key={c}>
                          <input value={String(row[c] ?? '')} onChange={e => updateCell(ri, c, e.target.value)} />
                        </td>
                      ))}
                      <td>
                        <button className={styles.removeBtn} onClick={() => removeRow(ri)} title="Remover linha">×</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
